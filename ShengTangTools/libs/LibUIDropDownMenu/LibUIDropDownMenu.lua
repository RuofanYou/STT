...
local MINOR_VERSION = 90000 + (tonumber(("$Rev$"):match("%d+")) or 0)
...
do
    ...
    -- 初始层级为 DIALOG，真正显示时会按拥有者动态下调
    L_DropDownList1:SetFrameStrata("DIALOG")
    ...
    L_DropDownList2:SetFrameStrata("DIALOG")
...
function L_ToggleDropDownMenu(level, value, dropDownFrame, anchorName, xOffset, yOffset, menuList, button, autoHideDelay)
	...
	if ( listFrame:IsShown() and (L_UIDROPDOWNMENU_OPEN_MENU == tempFrame) ) then
        ...
        -- 动态层级：跟随拥有者（下拉所属的父框架）以避免覆盖别的插件窗口
        local owner = dropDownFrame or L_UIDROPDOWNMENU_OPEN_MENU
        if owner and owner.GetFrameStrata then
            local strata = owner:GetFrameStrata() or "MEDIUM"
            listFrame:SetFrameStrata(strata)
            local lvl = (owner.GetFrameLevel and owner:GetFrameLevel() or 0) + 10
            listFrame:SetFrameLevel(lvl)
            listFrame:SetToplevel(false)
        end
